 ...
 */
export const lngLatToTileXY = (lng: number, lat: number, level: number) => {
 ...
 */
const lngLatToPixelXY = (lng: number, lat: number, level: number) => {
...
const getBingTileURL = async () => {
  let meta = (await getBingMeta()) as any;
  let url = meta.resourceSets[0].resources[0].imageUrl as string;
